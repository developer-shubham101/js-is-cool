function processData(input) {
  ...
  var patternOfALink = /<a.*?>(.*?)<\/a>/g;
  var stringList = input.match(patternOfALink);

  var patternOfLink = /<a (href\=\"(.*?)\"[^\"]?)\s?(title\=\".+\")?>(.*?)<\/a>/;
  var patternOfContainerTag = /<.[^>]*>(.*?)<\/.>/;
  var patternOfEmptyTag = /<[^/>][^>]*><\/[^>]+>/;

  for (const element of stringList) {
    if (patternOfLink.test(element)) {

      var link = element.match(patternOfLink)[2];
      var text = element.match(patternOfLink)[4];

      var extractText = '';
      if (patternOfEmptyTag.test(element)) {
        extractText = '';
      } else if (patternOfContainerTag.test(element)) {
        var extractTextList = text.match(patternOfContainerTag);
        extractText = extractTextList ? extractTextList[1] : text;
      }
      console.log(`${link},${extractText.trim()}`);
    }
